...
import { NailStudioResult } from "@/types/nail-studio";
...
interface NailResultsGalleryProps {
  results: NailStudioResult[];
  processingCount: number;
  onDownload: (result: NailStudioResult) => void;
  onRetry: (resultId: string) => void;
  onDownloadAll: () => void;
}

export function NailResultsGallery({
  results,
  processingCount,
  onDownload,
  onRetry,
  onDownloadAll,
}: NailResultsGalleryProps) {
  const t = useTranslations("nailStudio.results");
  const tc = useTranslations("common");
  const { user } = useUser();
  const { toast } = useToast();
  const pathname = usePathname();

  const [previewResult, setPreviewResult] = useState<NailStudioResult | null>(
    null
  );
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [fileToSave, setFileToSave] = useState<NailStudioResult | null>(null);
  ...
  const handleSave = useCallback(
    async (folderId: string | null) => {
      ...
      setIsSaving(true);
      try {
        const fileName = `nail-${fileToSave.type}-${fileToSave.nailImageName}-${Date.now()}.png`;
  ...
  const handleSaveAll = useCallback(
    async (folderId: string | null) => {
      ...
      for (const result of successfulResults) {
        ...
        try {
          const fileName = `nail-${result.type}-${result.nailImageName}-${Date.now()}.png`;
          const saved = await saveFileFromDataUrl(
            user.id,
            result.imageDataUrl,
            fileName,
    ...
    <>
              ...
              >
                {result.imageDataUrl ? (
                  <img
                    ...
                    alt={`${result.nailImageName} - ${result.targetImageName}`}
      ...
      >
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>
              {previewResult?.nailImageName} - {previewResult?.targetImageName}
            </DialogTitle>
          </DialogHeader>
